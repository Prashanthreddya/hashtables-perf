...
void randomid(long size,hash &ca1,hash &ca2,hash &ca3)
{
    ...
    for(int i=0;i<size;i++)
    {
        ...
        ca1.add_car(strid[i],strname[i],1);
        ca2.add_car(strid[i],strname[i],2);
        ca3.add_car(strid[i],strname[i],3);
        cout<<strid[i]<<"  ";
...
int main()
{
	...
	for(;;)
	{
		cout<<"\n\n1:Create hashtable with random entries | 2:Search for an ID | 3:Delete for an ID | 4:Compute efficiency | 5:Exit\n";
		cin>>ch;
		...
		switch(ch)
		{
                        ...
                        case 1:  long size;
                                 cout<<"Enter the number of random details you want to enter : ";
                                 cin>>size;
                                 randomid(size,ca1,ca2,ca3);   
